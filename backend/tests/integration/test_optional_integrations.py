"""Opt-in tests for the REAL external services (Phase 5 · 5H).

Skipped unless credentials are present, so CI stays hermetic while the real paths
stay exercisable on demand:
    GEMINI_API_KEY set                      → real Gemini judge
    ANCHOR_PROVIDER=evm + ANCHOR_EVM_*      → real Sepolia anchoring
Run e.g.:  GEMINI_API_KEY=... pytest backend/tests/integration/test_optional_integrations.py
"""
...
def test_real_gemini_grades_a_row(make_org, client):
    ...
    try:
        ...
        worker._grade_one(db, rows[0])                 # REAL network call to Gemini
...
def test_real_evm_anchor_runs(make_org, client):
    ...
    db = SessionLocal()
    try:
        n = anchor.anchor_all_due(db, get_settings())  # REAL Sepolia tx attempt
        assert n >= 0
    finally:
        db.close()
