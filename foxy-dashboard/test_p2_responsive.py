...
def test_every_destination_is_reachable_when_the_rail_collapses(html):
    """The rail hides at <=1180px. The bottom bar only carries five entries, so
    the drawer is the only route to the other six — it must exist and open."""
    rail_hidden = re.search(r"@media\(max-width:1180px\)\{(.*?)\n\}", html, re.S)
